...
#train$income<-droplevels(train$income)
test_random_forest_classification<-function(){
  train$income<-as.factor(train$income)

  random_forest_classifier<-randomForest(income ~ hours.per.week + age + fnlwgt + education.num + capital.gain + capital.loss, data=train,   na.action = na.roughfix)
